...
class erler_4_buttons extends ZigBeeDevice {
    async onNodeInit({ zclNode }) {
        ...
        this._buttonPressedTriggerDevice = this.homey.flow
            .getDeviceTriggerCard("erler_4_buttons")
            .registerRunListener(async (args, state) => {
                return (
                    null,
                    args.button === state.button && args.action === state.action
                );
            });
    }
    ...
    buttonCommandParser(ep, frame) {
        ...
        return this._buttonPressedTriggerDevice
            .trigger(this, {}, { button: `${button}`, action: `${action}` })
            .then(() =>
                this.log(
                    `Triggered 4 Button Remote, button=${button} action=${action}`
                )
            )
            .catch((err) =>
                this.error("Error triggering 4 button Remote", err)
            );
    }
...
module.exports = erler_4_buttons;
